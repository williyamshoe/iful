...
class IFULModel:
        ...
        while not_valid:
            ...
            for x in np.arange(self.sourceplane_size):
                for y in np.arange(self.sourceplane_size):
                    ...
                    v = self.sm._light_model.surface_brightness(
    ...
    def voronoi_given_nbins(self, target_y, low, high, kwargs_source, epsilon=1e-7):
        ...
        self.init_bin_sourceflux = self.sm._light_model.surface_brightness(
    ...
    def given_ra_dec_return_bin_no(
        ...
        res = find_closest_point_indices(
            np.array([self.x_bins, self.y_bins]).T, np.array([x_ra, y_dec]).T
        )
        res = np.array(
            [
                r if d <= self.threshold_source_radius else np.nan
                for r, d in zip(res, dists)
            ]
        )
    ...
    def gen_binned_lensed_whitelight(
        self, kwargs_source, kwargs_lens, binned_fluxes=None
    ):
        ...
        x_source, y_source = self.sm._lens_model.ray_shooting(
            ra_grid, dec_grid, kwargs_lens
        )

        inds = self.given_ra_dec_return_bin_no(
            x_source,
            y_source,
            kwargs_source[0],
            kwargs_source[0]["R_sersic"] * self.num_rsersics,
        )
        source_light = np.array(
            [0 if np.isnan(ind) else binned_fluxes[int(ind)] for ind in inds]
        )

        return self.immodel.ImageNumerics.re_size_convolve(
            source_light, unconvolved=False
        )
    ...
    def gen_binned_unlensed_whitelight(self, kwargs_source, binned_fluxes=None):
        ...
        for x in tqdm(np.arange(self.sourceplane_size)):
            for y in np.arange(self.sourceplane_size):
                ...
                ind = self.given_ra_dec_return_bin_no([xval], [yval], kwargs_source[0])[
                    0
                ]
                if np.isnan(ind):
                    source_light = np.nan
