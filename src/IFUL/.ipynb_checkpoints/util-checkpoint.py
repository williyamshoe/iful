...
def gen_gif(data_datacube, model_datacube, var_datacube, mask_3d, waves, name):
    ...
    for i, _ in enumerate(waves):
        ...
        plt.subplot(1, 3, 3)
        plt.imshow(((data_datacube-model_datacube) * mask_3d/var_datacube**0.5)[i, :, :], vmin=-6, vmax=6, cmap='bwr')
        plt.gca().set_axis_off()
...
def homography_loss(points_src, points_dst):
    ...
    cost_matrix = np.sum((p_src[:, np.newaxis, :] - p_dst[np.newaxis, :, :])**2, axis=2)
...
def check_near_bounds(results, lowers, uppers, ind):
    for k in results:
        if k in uppers and np.abs(results[k] - uppers[k]) <= 1e-3:
            print(f"profile {ind} parameter {k} near upper bound (value {results[k]}, bound {uppers[k]})")
        elif k in lowers and np.abs(results[k] - lowers[k]) <= 1e-3:
            print(f"profile {ind} parameter {k} near lower bound (value {results[k]}, bound {lowers[k]})")

def get_reduced_chi_sq(fitting_seq, kwargs_model, mask):
    lensPlot = ModelPlot(fitting_seq.multi_band_list, kwargs_model, fitting_seq.best_fit(), arrow_size=0.02, cmap_string="gist_heat",
                         linear_solver=kwargs_constraints.get('linear_solver', True), image_likelihood_mask_list=np.array([mask]))
...
def is_border_all_nan(arr_2d):
    ...
    border_elements = np.concatenate([top_border, bottom_border, left_border, right_border])
